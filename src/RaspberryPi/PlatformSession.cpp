 ...
 *
 * HonkordGL — Raspberry Pi PlatformSession (X11 + optional GLFW)
 * Copyright (C) 2026 Honkord
 */

#include "PlatformSession.hpp"

#if defined(HONKORDGL_PLATFORM_RASPBERRY_PI) && defined(HONKORDGL_PLATFORM_USES_X11_SOURCES)
...
bool PlatformSession::connect_display(const char * display_name) noexcept
{
    ...
    if (!dpy_) {
        HonkordGL::Graphics::SetInternalApiError("Raspberry Pi PlatformSession: XOpenDisplay failed.");
        return false;
    }
...
bool PlatformSession::prepare_glfw() noexcept
{
    ...
    if (glfwInit() != GLFW_TRUE) {
        HonkordGL::Graphics::SetInternalApiError("Raspberry Pi PlatformSession: glfwInit failed.");
        return false;
    }
    glfw_ok_ = true;
    return true;
...
#elif defined(HONKORDGL_PLATFORM_RASPBERRY_PI)
