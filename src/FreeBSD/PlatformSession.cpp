 ...
 *
 * HonkordGL — FreeBSD PlatformSession (X11 + optional GLFW)
 * Copyright (C) 2026 Honkord
 */

#include "PlatformSession.hpp"

#if defined(__FreeBSD__) && defined(HONKORDGL_PLATFORM_USES_X11_SOURCES)
...
bool PlatformSession::connect_display(const char * display_name) noexcept
{
    ...
    if (!dpy_) {
        HonkordGL::Graphics::SetInternalApiError("FreeBSD PlatformSession: XOpenDisplay failed.");
        return false;
    }
...
bool PlatformSession::prepare_glfw() noexcept
{
    ...
    if (glfwInit() != GLFW_TRUE) {
        HonkordGL::Graphics::SetInternalApiError("FreeBSD PlatformSession: glfwInit failed.");
        return false;
    }
    glfw_ok_ = true;
    return true;
