...
bool WindowBackend::BorrowFromSession(Internal::PlatformSession& session) noexcept
{
    if (!session.is_connected()) {
        HonkordGL::Graphics::SetInternalApiError("BorrowFromSession: X11 PlatformSession is not connected.");
        return false;
    }
    CloseDisplay();
...
bool WindowBackend::EnableRandrMonitorPolling() noexcept
{
    if (!display_ || !ext_xrandr_)
        return false;
    return Internal::RandrEnableMonitorPolling(display_, screen_);
}
bool WindowBackend::PollMonitorsChanged() noexcept
{
    if (!display_ || !ext_xrandr_)
        return false;
    return Internal::RandrPollMonitorTopologyChanged(display_, screen_, xrandr_event_base_);
}
int WindowBackend::GetMonitorCount() const noexcept
{
    if (!display_)
        return 0;
    return Internal::GetMonitorCount(display_, screen_);
}
bool WindowBackend::GetMonitorBounds(int index, Recti * out) const noexcept
{
    if (!display_ || !out)
        return false;
    return Internal::GetMonitorBounds(display_, screen_, index, out);
}
...
bool WindowBackend::CreateWindow(ApplicationContextSettings& ctx) noexcept
{
    ...
    ctx.needs_resize = 0;
    ctx.native_platform = static_cast<int>(NativePlatform::X11);
...
bool WindowBackend::CreateIpcHelper(IpcHelperWindow * out, const char * message_atom_name) const noexcept
{
    if (!display_ || !out)
        return false;
    if (!CreateIpcHelperWindow(reinterpret_cast<HonkordGL_GW_Handle>(display_), screen_, message_atom_name, out)) {
