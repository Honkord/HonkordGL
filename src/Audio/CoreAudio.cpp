/**
 * HonkordGL — Audio API implementation (Core Audio / Audio Unit on Apple platforms; stubs elsewhere)
    ...
    AudioBufferList * ioData)
{
    ...
    (void)inTimeStamp;
    (void)inBusNumber;
    auto * m = static_cast<MixerImpl *>(inRefCon);
    if (!m || m->isBufferMode || !ioData || ioData->mNumberBuffers < 1 || inNumberFrames == 0)
        return noErr;
    ...
    m->renderToOutputFloats(dst, inNumberFrames);
    return noErr;
}
static void disposeAudioUnit(AudioComponentInstance & au) noexcept
{
    if (!au)
        return;
    (void)AudioOutputUnitStop(au);
    (void)AudioUnitUninitialize(au);
    (void)AudioComponentInstanceDispose(au);
    au = nullptr;
}
static bool fillFloat32InterleavedASBD(AudioStreamBasicDescription & asbd, int sampleRate, int channels) noexcept
{
    if (sampleRate <= 0 || channels <= 0)
        return false;
    std::memset(&asbd, 0, sizeof(asbd));
    asbd.mSampleRate = static_cast<Float64>(sampleRate);
    asbd.mFormatID = kAudioFormatLinearPCM;
    asbd.mFormatFlags =
        static_cast<AudioFormatFlags>(kAudioFormatFlagIsFloat | kAudioFormatFlagIsPacked | kAudioFormatFlagsNativeEndian);
    asbd.mBytesPerPacket = static_cast<UInt32>(sizeof(float) * static_cast<std::size_t>(channels));
    asbd.mFramesPerPacket = 1;
    asbd.mBytesPerFrame = static_cast<UInt32>(sizeof(float) * static_cast<std::size_t>(channels));
    asbd.mChannelsPerFrame = static_cast<UInt32>(channels);
    asbd.mBitsPerChannel = 32;
    return true;
}
bool openAudioUnitOutput(MixerImpl & out, AudioFormat const & req, unsigned subsystemFlags) noexcept
{
...
#if TARGET_OS_IPHONE
    {
        ...
        if (AudioUnitSetProperty(
                au, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &enableOutput, sizeof(enableOutput))
            != noErr) {
            disposeAudioUnit(au);
            return false;
        }
        if (AudioUnitSetProperty(
                au, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1, &enableInput, sizeof(enableInput))
            != noErr) {
            disposeAudioUnit(au);
            return false;
        }
    ...
    if (!fillFloat32InterleavedASBD(asbd, rate, ch)) {
        disposeAudioUnit(au);
        return false;
    }

    if (AudioUnitSetProperty(
            au, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &asbd, sizeof(asbd))
        != noErr) {
        disposeAudioUnit(au);
        return false;
    }
    ...
    if ((subsystemFlags & static_cast<unsigned>(AudioSubsystemInit::LowLatency)) != 0u) {
        UInt32 const framesPerSlice = 256;
        (void)AudioUnitSetProperty(
            au, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, 0, &framesPerSlice, sizeof(framesPerSlice));
    }

    AURenderCallbackStruct cb{};
    cb.inputProc = coreAudioRenderCallback;
    cb.inputProcRefCon = &out;
    if (AudioUnitSetProperty(
...
int AudioMixer::Play() noexcept
{
    ...
    m->startWorker();
    if (m->audioUnit) {
        OSStatus const st = AudioOutputUnitStart(m->audioUnit);
        if (st != noErr)
...
void AudioMixer::Stop() noexcept
{
    ...
    m->stopWorker();
    if (m->audioUnit)
        (void)AudioOutputUnitStop(m->audioUnit);
}
...
AudioMixer * CreateAudioMixerDevice(char const * nameTag, AudioFormat const * request) noexcept
{
    ...
    if (!detail::openAudioUnitOutput(*mimpl, req, detail::g_subsysInitFlags)) {
        delete mimpl;
        return nullptr;
    }
...
void ObliterateAudioMixer(AudioMixer * mixer) noexcept
{
    ...
    mimpl->stopWorker();
    detail::disposeAudioUnit(mimpl->audioUnit);
