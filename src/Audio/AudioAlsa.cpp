/**
 * HonkordGL — Audio API implementation (ALSA on Linux desktop; stubs elsewhere)
...
struct MixerImpl {
    ...
    void workerLoop() noexcept
    {
        ...
        std::vector<std::int16_t> s16Scratch;
        std::vector<float> alsaFloat;
        ...
        while (!workerStop.load(std::memory_order_acquire)) {
            ...
            trackScratch.resize(sampleCount);

            {
                ...
                if (mixCb) {
                    ctx.trackIndex = -1;
                    (void)mixCb(AudioMixCallbackStage::PreMix, mix.data(), static_cast<std::uint32_t>(frameCount), ch, &ctx);
                }
                ...
                for (std::size_t ti = 0; ti < trackList.size(); ++ti) {
                    ...
                    for (std::size_t f = 0; f < frameCount; ++f) {
                        ...
                        for (int c = 0; c < bc; ++c) {
                            ...
                            trackScratch[f * static_cast<std::size_t>(ch) + static_cast<std::size_t>(c)] = s * vol;
                        }
                        t.readPos += static_cast<double>(t.pitch);
                    }

                    for (std::size_t s = 0; s < sampleCount; ++s)
                        mix[s] += trackScratch[s];

                    if (mixCb) {
                        ctx.trackIndex = static_cast<int>(ti);
                        (void)mixCb(AudioMixCallbackStage::PostTrack, mix.data(), static_cast<std::uint32_t>(frameCount), ch, &ctx);
                    }
                }

                if (mixCb) {
                    ctx.trackIndex = -1;
                    (void)mixCb(AudioMixCallbackStage::PostMix, mix.data(), static_cast<std::uint32_t>(frameCount), ch, &ctx);
                }
            }
            ...
            if (isBufferMode && userBuffer && userBufferBytes > 0) {
                ...
                if (format.sampleFormat == AudioSampleFormat::Float32Interleaved) {
                    ...
                    s16Scratch.resize(sampleCount);
                    FloatToS16(mix.data(), s16Scratch.data(), sampleCount);
                    std::memcpy(
                ...
                (void)snd_pcm_hw_params_current(pcm, hw);
                snd_pcm_format_t pf = SND_PCM_FORMAT_S16_LE;
                (void)snd_pcm_hw_params_get_format(hw, &pf);

                if (format.sampleFormat == AudioSampleFormat::Float32Interleaved || pf == SND_PCM_FORMAT_FLOAT_LE) {
                    alsaFloat.resize(sampleCount);
                    for (std::size_t i = 0; i < sampleCount; ++i)
                        alsaFloat[i] = mix[i];
                    snd_pcm_sframes_t w = snd_pcm_writei(pcm, alsaFloat.data(), static_cast<snd_pcm_sframes_t>(frameCount));
                    if (w < 0)
                        w = snd_pcm_recover(pcm, static_cast<int>(w), 0);
                } else {
                    s16Scratch.resize(sampleCount);
                    FloatToS16(mix.data(), s16Scratch.data(), sampleCount);
                    snd_pcm_sframes_t w = snd_pcm_writei(pcm, s16Scratch.data(), static_cast<snd_pcm_sframes_t>(frameCount));
                    if (w < 0)
                        w = snd_pcm_recover(pcm, static_cast<int>(w), 0);
                }
            }
...
bool openPcm(MixerImpl & out, AudioFormat const & req) noexcept
{
    int rate = req.sampleRateHz > 0 ? req.sampleRateHz : 48000;
    int ch = req.channelCount > 0 ? req.channelCount : 2;
    snd_pcm_format_t pcmFmt = SND_PCM_FORMAT_FLOAT_LE;
    int err = snd_pcm_open(&out.pcm, "default", SND_PCM_STREAM_PLAYBACK, 0);
    if (err < 0) {
        out.pcm = nullptr;
        return false;
    }
    ...
    out.format.sampleFormat = (pcmFmt == SND_PCM_FORMAT_FLOAT_LE) ? AudioSampleFormat::Float32Interleaved
...
/* Non-Linux: stub implementations so the TU links everywhere. */
