...
bool WindowBackend::BorrowFromSession(Internal::PlatformSession& session) noexcept
{
    if (!session.is_connected()) {
        HonkordGL::Graphics::SetInternalApiError("BorrowFromSession: NetBSD PlatformSession is not connected.");
        return false;
    }
    CloseDisplay();
...
bool WindowBackend::CreateWindow(ApplicationContextSettings& ctx) noexcept
{
    ...
    ctx.needs_resize = 0;
    ctx.native_platform = static_cast<int>(NativePlatform::NetBSD);
...
namespace HonkordGL::Graphics::NetBSD 
{
void NetBSDWindowBackendCompileUnitPlaceholder() noexcept {}
}
